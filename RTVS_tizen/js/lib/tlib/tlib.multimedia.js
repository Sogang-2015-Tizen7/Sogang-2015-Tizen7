...
if (typeof tlib.multimedia === "undefined") {
    tlib.multimedia = function() {
        ...
        return {
             ...
             *
             * @param optionsInit {Object} List of options to initialize
             *        playback
             */
            initialize : function(optionsInit) {
                ...
                if (options.type === types.AUDIO) {
                    page.append('<audio id="' + elementName
                            + '" controls="controls"></audio>');
                    page.find('#controls').hide();
                } else {
                    page.append('<video id="' + elementName
                            + '" width="360"></video>');
                ...
                if (multimediaInstanceJQ.find("source").length === 0) {
                    ...
                    multimediaInstanceJQ.find("source").attr('src',
                            options.file);
            ...
            stop : function() {
                try {
                    ...
                    console.error("Unable to stop the Video!");
                }
            },
             ...
             * Key binders method
             *
             * @param elements {Object} JSON stores all available keys for which
             *        the actions can be hooked
             */
             ...
             *
             * @param flag {Boolean} If true we turn one fullscreen mode
             *        otherwise we turn it off
             */
            fullscreen : function(flag) {
                console.info("Set fullscreen -> " + flag);
                var that = this;
                if (flag && options.type === types.VIDEO) {
                    try {
                        multimediaInstance.webkitRequestFullScreen();
                        /**
                         * When we enter fullscreen mode we should handle some
                         * event to allow user to leave fullscreen mode, because
                         * browser doesn't do it automatically. We use either
                         * swipeLeft or swipeRight for this.
                         */
                        $(document).bind('webkitfullscreenchange', function() {
                            $('html').bind({
                                swipeleft : function() {
                                    console.info("In FullScreen swipe left");
                                    that.fullscreen(false);
                                },
                                swiperight : function() {
                                    console.info("In FullScreen swipe right");
                                    that.fullscreen(false);
                                }
                            });
                        });
                    } catch (e) {
                        console.warn("Unable to start fullscreen");
                    }
                } else {
             ...
             *
             * @param errorCB {Function} Callback invoked in case of playback
             *        error
